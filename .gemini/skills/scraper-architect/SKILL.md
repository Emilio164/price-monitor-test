...
3.  **Patrón de Playwright Estándar (Sigilo Obligatorio)**:
    *   Usa `headless=True`.
    *   **Headers**: Configura `User-Agent` moderno, `Accept-Language: es-AR...` y `Referer: https://www.google.com/`.
    *   **Delays Humanos**: Añade `asyncio.sleep(random.uniform(1.0, 3.0))` antes de navegar.
    *   **Interacción**: Realiza un `page.mouse.wheel(0, 500)` después de cargar para simular lectura y activar lazy-loading.
